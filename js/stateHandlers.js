...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            this.attributes['beatsList'] = Array.apply(null, {length: audioData.length}).map(Number.call, Number);
            this.attributes['index'] = Math.floor(Math.random() * this.attributes['beatsList'].length);
            this.attributes['offsetInMilliseconds'] = 0;
            this.attributes['loop'] = true;
            this.attributes['shuffle'] = false;
            this.attributes['playbackIndexChanged'] = true;
            this.attributes['numRappers'] = 1;
            //  Change state to START_MODE
            this.handler.state = constants.states.START_MODE;

            var message = 'Yo yo yo, welcome to freestyle, just ask me to drop the beat, if your not feeling it, just say next';
            var reprompt = 'You can say, drop the beat, to begin.';
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'Welcome to the freestyle, say drop the beat to begin';
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'AMAZON.HelpIntent' : function () {
            ...
            var message = 'You are playing freestyle. You can say, Next or Previous to navigate through the playlist. ' +
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
         ...
         */
        'LaunchRequest' : function () {
            var message = 'You were listening to ' + audioData[this.attributes['beatsList'][this.attributes['index']]].title +
...
var controller = function () {
    return {
        play: function () {
            ...
            var token = String(this.attributes['beatsList'][this.attributes['index']]);
            var playBehavior = 'REPLACE_ALL';
            var beat = audioData[this.attributes['beatsList'][this.attributes['index']]];
            var offsetInMilliseconds = this.attributes['offsetInMilliseconds'];
            // Since play behavior is REPLACE_ALL, enqueuedToken attribute need to be set to null.
            this.attributes['enqueuedToken'] = null;

            if (canThrowCard.call(this)) {
                var cardTitle = 'Playing ' + beat.title;
                var cardContent = 'Playing ' + beat.title;
                this.response.cardRenderer(cardTitle, cardContent, null);
            }

            this.response.audioPlayerPlay(playBehavior, beat.url, token, null, offsetInMilliseconds);
            this.emit(':responseReady');
        },
        stop: function () {
            /*
        ...
        playNext: function () {
             ...
             */
            var index =  Math.floor(Math.random() * this.attributes['beatsList'].length);
        ...
        shuffleOn: function () {
            ...
            shuffleOrder((newOrder) => {
                ...
                this.attributes['beatsList'] = newOrder;
                this.attributes['index'] = 0;
                this.attributes['offsetInMilliseconds'] = 0;
                this.attributes['playbackIndexChanged'] = true;
                controller.play.call(this);
        ...
        shuffleOff: function () {
            ...
            if (this.attributes['shuffle']) {
                this.attributes['shuffle'] = false;
                // Although changing index, no change in audio file being played as the change is to account for reordering beatsList
                this.attributes['index'] = this.attributes['beatsList'][this.attributes['index']];
                this.attributes['beatsList'] = Array.apply(null, {length: audioData.length}).map(Number.call, Number);
            }
            controller.play.call(this);
        },
